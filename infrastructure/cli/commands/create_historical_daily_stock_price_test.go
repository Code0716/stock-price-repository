...
func TestCreateHistoricalDailyStockPricesV1Command_Action(t *testing.T) {
	type fields struct {
	...
	}{
		{
			...
			fields: fields{
				stockBrandsDailyStockPriceInteractor: func(ctrl *gomock.Controller) usecase.StockBrandsDailyPriceInteractor {
					...
					mock.EXPECT().CreateHistoricalDailyStockPrices(gomock.Any(), gomock.Any()).Return(nil)
					return mock
				},
			},
			args: args{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c := &CreateHistoricalDailyStockPricesV1Command{
			...
			if err := c.Action(tt.args.ctx); (err != nil) != tt.wantErr {
				t.Errorf("CreateHistoricalDailyStockPricesV1Command.Action() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
