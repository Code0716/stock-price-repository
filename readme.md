...
| **APIs**     | `J_QUANTS_BASE_URL_V2`                  | j-Quants API V2 ベース URL          |
...
毎日実行する定期バッチをまとめて実行します。場引け後（夕方〜夜）に1日1回実行します。

```bash
chmod +x shell_scripts/every.sh   # 初回のみ
./shell_scripts/every.sh
```

実行されるコマンドと順序（`shell_scripts/every.sh` の `DAILY_COMMANDS`）:

1. `update_stock_brands_v1` — 銘柄マスタ更新（新規上場・上場廃止を反映）
2. `create_daily_stock_price_v1` — 今日の日足取得（銘柄マスタに依存。最重要）
3. `grade_quiz_answers_v1` — クイズ回答の採点（日足に依存。出題作成より先に実行し、当日分の採点を早く確定させる）
...
> **Note**: クイズの採点・出題はこのバッチでのみ行われます（API サーバーは行いません）。バッチが実行されないと、クイズの結果確定も新規出題もされません。
...
出来高がありよく動く300銘柄（直近20営業日の平均売買代金上位600→平均値幅率上位300）を選定し、その日のクイズ出題を作成します（`create_daily_stock_price_v1` の後、当日終値取得後に実行。[毎日バッチ（every.sh）](#毎日バッチeverysh) に含まれる）。
...
翌営業日の終値が確定した未採点のクイズ回答を採点します（`create_daily_stock_price_v1` の後に実行。`create_quiz_daily_universe_v1` より先に実行し、その日の採点を早く確定させる。[毎日バッチ（every.sh）](#毎日バッチeverysh) に含まれる）。
