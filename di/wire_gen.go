...
func InitializeCli(ctx context.Context) (*cli.Runner, func(), error) {
	...
	finStatementRepository := database.NewFinStatementRepositoryImpl(gormDB)
	...
	dailyAvoidStockRepository := database.NewDailyAvoidStockRepositoryImpl(gormDB)
	createDailyAvoidStocksInteractor := usecase.NewCreateDailyAvoidStocksInteractor(transaction, adjustedDailyPriceRepository, stockBrandRepository, dailyAvoidStockRepository)
	createDailyAvoidStocksV1Command := commands.NewCreateDailyAvoidStocksV1Command(createDailyAvoidStocksInteractor)
	runner := cli.NewRunner(healthCheckCommand, updateStockBrandsV1Command, createHistoricalDailyStockPricesV1Command, createDailyStockPriceV1Command, rebuildAnalyzeDailyPricesV1Command, createNikkeiAndDjiHistoricalDataV1Command, adjustHistoricalDataForStockSplitCommand, adjustHistoricalDataForStockConsolidationCommand, exportYearlyDataCommand, exportMasterDataCommand, syncFinAnnouncementsCommand, syncFinStatementsCommand, backtestAllStocksCommand, syncFinStatementsAllStocksCommand, gradeQuizAnswersV1Command, createQuizDailyUniverseV1Command, evaluateDailyStockPicksV1Command, createDailyStockPicksV1Command, createDailyAvoidStocksV1Command, indexInteractor, slackAPIClient)
	return runner, func() {
		cleanup()
	}, nil
}
...
func InitializeApiServer(ctx context.Context) (*http.ServeMux, func(), error) {
	...
	sectorPerformanceInteractor := usecase.NewSectorPerformanceInteractor(sector33AverageDailyPriceRepository, sector17AverageDailyPriceRepository)
	...
	dailyAvoidStockRepository := database.NewDailyAvoidStockRepositoryImpl(gormDB)
	dailyAvoidStockInteractor := usecase.NewDailyAvoidStockInteractor(dailyAvoidStockRepository, stockBrandRepository)
	dailyAvoidStockHandler := handler.NewDailyAvoidStockHandler(dailyAvoidStockInteractor, httpServer, logger)
